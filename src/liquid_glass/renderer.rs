...
pub struct LiquidGlassRenderer {
    ...
    /// Strong backdrop blur used only by the prominent settings panel. It is
    /// generated by running the already-blurred capture through the pyramid a
    /// second time, so no sharp center sample survives.
    prominent_blur_texture: wgpu::Texture,
    ...
    prominent_settings_panel_final_bind_group: wgpu::BindGroup,
    ...
    blur_dirty: bool,
    /// Set when the settings panel starts using its dedicated capture blur or
    /// when textures/parameters change while that pass is inactive.
    prominent_blur_dirty: bool,
...
impl LiquidGlassRenderer {
    pub fn new(
        ...
        let (prominent_blur_texture, prominent_blur_view) =
            create_blur_texture_raw(device, width, height, 0, "prominent capture blur texture");
        ...
        let prominent_settings_panel_final_bind_group = create_final_bind_group(
            device,
            &final_bind_group_layout,
            &settings_panel_uniform_buffer,
            &backdrop_view,
        ...
        Self {
            ...
            prominent_settings_panel_final_bind_group,
    ...
    fn rebuild_final_bind_groups(
        ...
        self.prominent_settings_panel_final_bind_group = create_final_bind_group(
            device,
            &self.final_bind_group_layout,
            &self.settings_panel_uniform_buffer,
            backdrop_view,
    ...
    fn configure_cpu_backdrop(&mut self, device: &wgpu::Device, frame: &CpuCaptureFrame) -> bool {
        ...
        if texture_changed {
            ...
            let (prominent_blur_texture, prominent_blur_view) = create_blur_texture_raw(
                device,
                frame.width,
                frame.height,
                0,
                "prominent capture blur texture",
            );
            let blur_levels = [
    ...
    pub fn set_modal_shapes(
        ...
        let activates_prominent_blur = material_strength > 0.001
            && !shapes.is_empty()
            && (self.settings_panel_shapes.is_empty()
                || self.settings_panel_material_strength <= 0.001);
...
fn should_refresh_prominent_blur(active: bool, dirty: bool, normal_blur_refreshed: bool) -> bool {
    active && (dirty || normal_blur_refreshed)
}
...
mod shape_capacity_tests {
    use super::{
        ...
        overlay_shape_may_affect_clip, should_refresh_blur, should_refresh_prominent_blur,
        CaptureRegion, GlassShape, GlassUniforms,
    };
    ...
    #[test]
    fn prominent_capture_blur_runs_only_while_settings_are_active() {
        assert!(should_refresh_prominent_blur(true, true, false));
        assert!(should_refresh_prominent_blur(true, false, true));
        assert!(!should_refresh_prominent_blur(true, false, false));
        assert!(!should_refresh_prominent_blur(false, true, true));
    }
