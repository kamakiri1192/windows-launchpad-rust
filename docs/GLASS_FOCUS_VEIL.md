...
追加平滑化します。さらにパネルが直接参照するOS背景キャプチャも、既存の
Dual-Kawase結果へ40 px広域の9タップを追加します。パネル内のシャープな背景・
反射サンプルも同じブラーへ切り替えるため、モーダル描画時に未加工のキャプチャが
再び混ざることはありません。淡いtintと組み合わせ、通常のフォルダよりも曇り
ガラスに近い見た目にしています。
...
- 設定パネル内の下層シーン追加ブラー半径は `src/renderer/frame.rs` の
  `PROMINENT_FOCUS_BLUR_SPREAD`、OS背景キャプチャの追加ブラー半径は
  `src/liquid_glass/renderer/frame.rs` の `SETTINGS_CAPTURE_BLUR_SPREAD` で調整します。
