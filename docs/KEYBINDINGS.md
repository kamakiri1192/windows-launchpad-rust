...
> **設定画面からの操作（issue #112）:** `V`、数値5種、`M`、`R`、
> `B/G/D/A/F/C/E/L` のデバッグ操作は、**設定パネルの「デバッグ」カテゴリ**
> からも全く同じように行えます。設定パネルは **`debug_keys_enabled`
> トグルの状態によらず常に操作可能** です（キーボードショートカットを
> 有効にしていなくても使えます）。
>
> 永続化の挙動:
> - **永続化される**（再起動後も維持）: `V`（有効/無効）と数値5種
>   （thickness, refractive_index, saturation, chromatic_aberration,
>   blur_radius）。
> - **セッションのみ**（再起動でリセット）: `M`（ウィンドウ装飾）と
>   `B/G/D/A/F`（デバッグビュー）と `C/E/L`（無効化フラグ）。これらは
>   誤ってデバッグ状態を残したまま終了しても影響が残らないよう保存されません。
> - 各数値パラメータの行には **↺ アイコン（個別リセット）** があり、
>   そのパラメータだけをコード既定値に戻せます。セクション末尾の
>   **「Liquid Glass をデフォルトに戻す」** ボタンで V 含む全パラメータを
>   一括リセットできます。
> - 設定パネルの Debug カテゴリは内容が多いので**縦スクロール**します
>   （ホイール / トラックパッド、または上下のヒットストリップ）。
