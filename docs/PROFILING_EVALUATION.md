...
8. `invalid_samples` が多いrunはtimestamp結果を信用せず、再計測する。

### macOS の再利用可能な計測コマンド

macOSでは、release build、Metal指定、必須シナリオ各3回の決定的QA、実ウィンドウの
CPU/RSS採取、ログとGPU profileの集約を次のスクリプトで統一できます。

~~~sh
scripts/profile_macos.sh qa
scripts/profile_macos.sh folder-scroll
DURATION_SECONDS=30 scripts/profile_macos.sh live
scripts/profile_macos.sh gpu
scripts/profile_macos.sh scroll
scripts/profile_macos.sh scroll-gpu
scripts/profile_macos.sh edit
scripts/profile_macos.sh edit-gpu
CAPTURE_SCALE=1 DURATION_SECONDS=30 scripts/profile_macos.sh live
~~~

結果は `target/macos-profile-*` の `summary.md` と `summary.json` にまとまります。
QAはPNG readback込みの比較用、liveは60 Hzの動画相当背景とScreenCaptureKitを
含む通常releaseの実負荷確認用です。gpuは描画区間別のtimestamp queryを有効に
する詳細調査用で、計測自体がframe pacingへ影響し得るためliveの代用にはしません。
folder-scrollは18個の子アプリを持つ2ページのフォルダを60fpsで3回往復し、
`summary`へフレーム時間のp95、スクロール中のp95、再レイアウト回数、
プロセスのreal/user/system CPU時間を集約します。
PNG readbackを含むため絶対値ではなく、同じ環境・viewportでの変更前後比較に使います。
scrollは実ウィンドウをページ間で3秒周期に往復させる通常releaseの継続負荷、
scroll-gpuは同じ負荷にtimestamp queryを加えたボトルネック調査用です。
editはproductionと同じ編集モードの揺れ・削除バッジ・操作コントロールを継続描画する
通常releaseの負荷、edit-gpuは同じ編集状態の描画区間別調査用です。
最後のコマンドはROIを物理解像度と同じ幅・高さで取得する対照測定です。通常、
表示倍率が2倍のRetina環境では、キャプチャテクスチャの幅と高さをROIの物理
ピクセル寸法の50%にします（総画素数は75%減）。summaryにはwindow / ROI /
output解像度、目標更新頻度、full-window比のpixel削減率が記録されます。

live/gpu/scroll/scroll-gpu/edit/edit-gpuは既定で非アクティブ時にも60 Hzを維持する背景アニメーションを起動します。
実際の静止デスクトップを測る場合だけ`ANIMATED_BACKDROP=0`を指定してください。
アプリ表示中の背景取得には意図的な低頻度更新を入れていません。
...
6. 使用率が低いのに動きだけ不自然なら、`scripts/profile_macos.sh folder-scroll` のsummaryとmanifestで入力、Dragging、Settling、Idle、再レイアウト回数を確認する。
...
## 2026-07-22 のmacOS編集モード計測

計測環境はMacBook Air（Apple M3 / Metal 3）、ウィンドウ2560×1602物理ピクセル、
表示倍率2倍、60 Hzの動画相当背景です。トップレベル約177項目を編集状態にしたまま、
背景ストリームと描画側の取り込みをどちらも60 Hz設定で継続しました。

| 実装 | 背景stream fps | 描画側capture fps | フレーム置換（2秒平均） | edit badge glass p95 |
|---|---:|---:|---:|---:|
| 全ページのバッジを全画面SDFで評価 | 60.01 | 10.81 | 98.86 | 1.5375 ms |
| 現在ページへ影響するバッジだけに絞り込み | 60.03 | 44.84 | 30.43 | 0.7413 ms |
| 可視バッジごとの境界矩形だけを描画 | 60.01 | 60.00 | 0.00 | 0.3362 ms |

最終実装では、離れて配置される小さな削除バッジ同士に全画面の滑らかな和集合
（smooth union）は不要という
形状特性を利用しています。現在のページ枠に届かないバッジをCPUで除外したうえで、可視
バッジごとに小さな矩形を描き、その範囲内だけでLiquid GlassのSDFと屈折情報を計算します。
背景取得、ぼかし、編集アニメーションは毎フレーム更新のままで、時間方向の間引きは
行っていません。

通常releaseの再測定3回では、描画側capture fpsは59.93 / 60.00 / 60.00でした。
また、タイル背景・アイコン・削除バッジは、CPUバッファを作った時刻を個別位相へ混ぜず、
固定の項目別オフセットと単一の毎フレーム時刻を共有します。非同期のアイコン取得で
アイコン用バッファだけが更新されても、各レイヤーの揺れはずれません。
