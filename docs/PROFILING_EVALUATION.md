...
QAはPNG readback込みの比較用、liveはScreenCaptureKit込みの実負荷確認用です。
最後のコマンドはROIを物理解像度1:1で取得する対照測定です。通常はRetinaの
scale factorから「1論理pointにつき1背景sample」を自動選択し、summaryには
window / ROI / output解像度とfull-window比のpixel削減率も記録されます。
